function tone=MakeGapInTone(varargin)



global pref

% Creates a gap stimulus consisting of a continuous  tone with a gap. 
%The tone is generated by MakeTone, see that function for
%params to use.
%
...
pregap_samples=pregap*samplerate/1000;
tone_params=params;
tone=MakeTone(tone_params, samplerate);
...
%Gap ramp
     ...
     if 2*ledge>gapdur_samples
         error('MakeGapInTone: Gap ramp too long for Gap duration ')
     end
     tone(gapstart+1:gapstart+ledge)=tone(gapstart+1:gapstart+ledge).*edge;
     tone(gapstart+ledge:gapstop-ledge)=  0*   tone(gapstart+ledge:gapstop-ledge); %silence in Gap
     tone((gapstop-ledge+1):gapstop)=tone((gapstop-ledge+1):gapstop).*fliplr(edge);
